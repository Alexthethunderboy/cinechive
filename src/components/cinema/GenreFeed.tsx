...
export function GenreFeed({ genreId, initialMovies, initialTv }: GenreFeedProps) {
  ...
  const {
    ...
    queryKey: ['genreFeed', 'movie', genreId],
    queryFn: async ({ pageParam = 1 }) => {
      if (pageParam === 1) return initialMovies;
      return await getGenrePageAction(genreId, 'movie', pageParam);
    },
    initialPageParam: 1,
    initialData: {
      pages: [initialMovies],
  ...
  const {
    ...
    queryKey: ['genreFeed', 'tv', genreId],
    queryFn: async ({ pageParam = 1 }) => {
      if (pageParam === 1) return initialTv;
      return await getGenrePageAction(genreId, 'tv', pageParam);
    },
    initialPageParam: 1,
    initialData: {
      pages: [initialTv],
