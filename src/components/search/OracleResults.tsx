...
import { Film, User, Music, ChevronRight } from 'lucide-react';
  ...
  if (!hasResults) {
    return (
      <GlassPanel className="p-12 text-center border-white/10 bg-black/80 backdrop-blur-2xl">
        <p className="text-muted font-heading text-lg italic">No frequencies detected in the archive.</p>
      </GlassPanel>
    );
  }
    ...
    <GlassPanel className="p-6 border-white/10 bg-black/90 backdrop-blur-3xl shadow-2xl max-h-[80vh] overflow-y-auto custom-scrollbar">
      <div className="space-y-10">
        ...
        {results.people.length > 0 && (
          <section>
            <Header icon={<User size={18} />} title="PERSONNEL" count={results.people.length} />
            <div className="flex gap-4 overflow-x-auto pb-4 pt-2 -mx-2 px-2 mask-linear-right custom-scrollbar">
              {results.people.map((person, idx) => (
                ...
                >
                  <div className="w-24 text-center">
                    <div className="w-20 h-20 mx-auto rounded-full overflow-hidden border-2 border-white/5 group-hover:border-vibe-violet transition-colors mb-2">
                       {person.profileUrl ? (
  ...
  return (
    <div className="flex items-center justify-between mb-4 border-b border-white/5 pb-2">
      <div className="flex items-center gap-2">
        <span className="text-white/60/80">{icon}</span>
